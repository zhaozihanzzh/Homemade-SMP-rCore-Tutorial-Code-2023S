...
impl<R: gimli::Reader> Context<R> {
    ...
    {
        let mut units_iter = self.find_units_range(probe, probe + 1);//self.find_units(probe);
        if let Some((unit, range)) = units_iter.next() {
...
pub struct Frame<'ctx, R: gimli::Reader> {
    ...
    /// Range from ? zzh added
    pub range: Option<gimli::Range>,
}
